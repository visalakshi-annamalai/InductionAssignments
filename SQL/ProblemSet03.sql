...
sselect year from movie m join ratings r on m.mID=r.mID where stars>=5 order by year;
...
select name from ratings r join movie m on m.mID=r.mID join reviewer r1
on r1.rID=r.rID where title='Gone with Wind';
...
select distinct name, title, stars from Reviewer, Movie, Rating 
where Reviewer.rID = Rating.rID  and Rating.mID = Movie.mID and name = director; 
...
select title from movie m join ratings r on m.mID=r.mID join reviewer r1
 on  r1.rID=r.rID where name not in(select name from reviewer where name='Chris
Jackson');
...
select name, title, stars from Reviewer, Movie, Ratings 
where stars = ( select min(stars) from Ratings) and Reviewer.rID = Ratings.rID  ; 
...
select title, rate from Movie m, ( select mID, avg(stars) as rate from ratings group by mID ) AvgRating 
where m.mID = AvgRating.mID 
...
select name from reviewer r join (select rID, count(stars)  from ratings group by rID ) r1 
on r.rID =r1.rID and r1.count(stars) >= 3; 
...
select M1.title, M1.director from Movie M1, Movie M2 where M1.director = M2.director and M1.title <> M2.title 
